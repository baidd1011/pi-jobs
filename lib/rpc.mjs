...
import { redactSensitive, safeError } from "./redact.mjs";
...
function spawnPi(config, args, cwd, envPatch = {}) {
...
export function resolveExecutionPolicy(task, config = {}) {
  const requested = task.policy || {};
  const tools = [...new Set(requested.tools || task.tools || config.tools || DEFAULT_TOOLS)];
  const noNetwork = Boolean(requested.noNetwork ?? task.noNetwork ?? false);
  const maxTurns = Number(requested.maxTurns ?? task.maxTurns ?? config.maxTurns ?? 200);
  if (!tools.length || tools.some((tool) => !ALLOWED_TOOLS.has(tool))) throw new Error("invalid execution tool policy");
  if (noNetwork && tools.includes("bash")) throw new Error("local-tools-only policy cannot enable bash");
  if (!Number.isSafeInteger(maxTurns) || maxTurns < 1 || maxTurns > 1000) throw new Error("invalid max-turns policy");
  return { tools, noNetwork, maxTurns };
}

export function buildPiInvocation(task, config = {}) {
  const policy = resolveExecutionPolicy(task, config);
  const args = ["--mode", "rpc", "--no-extensions", "--tools", policy.tools.join(",")];
  if (policy.noNetwork) args.push("--offline");
  const provider = task.provider ?? config.provider;
  const model = task.model ?? config.model;
  if (provider) args.push("--provider", provider);
  if (model) args.push("--model", model);
  return { args, env: policy.noNetwork ? { PI_OFFLINE: "1" } : {}, policy };
}
...
export function runPiTask(task, config, log, hooks = {}) {
  return new Promise((resolve) => {
    let invocation;
    try { invocation = buildPiInvocation(task, config); }
    catch (error) { resolve({ status: "failed", cost: 0, error: safeError(error) }); return; }

    const startedMs = Date.now();
    const child = spawnPi(config, invocation.args, task.worktreePath || task.cwd || task.repoRoot, invocation.env);
    const state = {
    ...
    const maxTurns = invocation.policy.maxTurns;
    ...
    const finish = (status, error) => {
      ...
      resolve({
        ...
        summary: state.lastText.slice(0, 500), error: error == null && state.llmError == null ? null : redactSensitive(error ?? state.llmError),
      });
    };
    ...
    const reportUsage = () => {
      ...
      catch (error) { log?.(`could not persist usage: ${safeError(error)}`); }
    };
    ...
    const requestStop = (candidate) => {
      ...
      if (changed) {
        ...
        catch (error) { log?.(`could not persist stop request: ${safeError(error)}`); return false; }
    ...
    child.on("error", (error) => finish("failed", `spawn failed: ${safeError(error)}`));
    child.on("exit", (code) => {
      if (done) return;
