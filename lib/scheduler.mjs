...
import {
  DATA_DIR, HEARTBEATS_DIR, TERMINAL_STATES, atomicWriteJson, clearHeartbeat,
  ensureDirs, inspectRunnerLock, isHeartbeatStale, isValidJobId, listJobs, loadConfig, readJob,
} from "./store.mjs";

export const TASK_NAME = "pi-jobs-worker";
export const LEGACY_TASK_NAME = "pi-nightshift";
...
function readPiSettings() {
  try { return JSON.parse(readFileSync(join(homedir(), ".pi", "agent", "settings.json"), "utf8")); }
  catch { return {}; }
}
...
export function doctor() {
  ...
  const taskOk = task.ok && task.detail.MultipleInstances === "Queue" && task.detail.Enabled
    ...
    && String(task.detail.Arguments).includes(RUNNER_PATH)
  ...
  try {
    for (const file of readdirSync(HEARTBEATS_DIR).filter((name) => name.endsWith(".json"))) {
      ...
      if (!job || TERMINAL_STATES.has(job.state)) { clearHeartbeat(id); cleanedHeartbeats.push(id); }
    }
  } catch {}
  checks.push({ name: "Terminal heartbeats", ok: true, detail: cleanedHeartbeats.length ? `cleaned ${cleanedHeartbeats.join(", ")}` : "none" });
