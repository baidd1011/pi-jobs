...
import {
  ...
  ensureDirs, inspectConfig, inspectQueueAudit, inspectRunnerLock, isHeartbeatStale, isValidJobId, readJob, scanJobRecords,
} from "./store.mjs";
...
export function scheduledTaskInfo() {
  ...
  catch (error) { return { ok: false, detail: safeError(error) || "task not registered" }; }
}
...
export function doctor({ cwd = null } = {}) {
  let dirsError = null;
  try { ensureDirs(); } catch (error) { dirsError = safeError(error); }
  ...
  checks.push({
    name: "Configuration",
    ok: configInspection.ok,
    detail: configInspection.ok
      ? `${configInspection.path}${configInspection.exists ? "" : " (not created; defaults would be used)"}`
      : `${configInspection.path}: ${configInspection.errorType} — ${configInspection.error}; repair the JSON manually or restore a known-good copy (doctor did not modify it)`,
  });
  ...
  } catch (error) { worktree = { ok: false, detail: safeError(error) }; }
  ...
  try {
    if (dirsError) throw new Error(dirsError);
    writeFileSync(probe, "ok"); unlinkSync(probe); checks.push({ name: "Data directory", ok: true, detail: DATA_DIR });
  } catch (error) { checks.push({ name: "Data directory", ok: false, detail: safeError(error) }); }

  const runnerLock = inspectRunnerLock();
  checks.push({ name: "Runner lock", ok: runnerLock.state !== "stale", detail: runnerLock.detail });

  let jobScan = { jobs: [], issues: [] };
  try { jobScan = scanJobRecords(); }
  catch (error) { jobScan.issues.push({ path: DATA_DIR, detail: safeError(error) }); }
  checks.push({
    name: "Job records",
    ok: jobScan.issues.length === 0,
    detail: jobScan.issues.length ? `${jobScan.issues.length} unhealthy: ${jobScan.issues.map((issue) => issue.path).join(", ")}` : `${jobScan.jobs.length} healthy`,
  });
  const staleJobs = jobScan.jobs.filter((job) => job.state === "running" && isHeartbeatStale(job.id)).map((job) => job.id);
  checks.push({ name: "Stale jobs", ok: staleJobs.length === 0, detail: staleJobs.length ? staleJobs.join(", ") : "none" });
  const activePaths = new Set(jobScan.jobs.filter((job) => job.state === "running" && job.worktreePath).map((job) => resolve(job.worktreePath).toLowerCase()));
  let worktreeDirs = [];
  ...
  const queueAudit = inspectQueueAudit();
  checks.push({
    name: "Queue audit",
    ok: queueAudit.missingRevisions.length === 0,
    detail: queueAudit.missingRevisions.length
      ? `missing revision(s): ${queueAudit.missingRevisions.join(", ")} — queue-events.jsonl was not modified`
      : queueAudit.stateRevision ? `revisions 1..${queueAudit.stateRevision} accounted for` : "no queue transitions recorded",
  });
  if (cwd) {
    let repoRoot = null;
