...
function isInside(child, parent) {
  ...
  const a = caseFold(normalizedChild);
  const b = caseFold(normalizedParent);
  if (a === b) return true;
...
function classifyWorktrees(config) {
  ...
  try {
    if (config?.worktreeRoot) worktreeRoot = resolve(String(config.worktreeRoot));
  } catch {}
  if (!worktreeRoot) {
...
function classifyOneWorktree(path, worktreeRoot) {
  ...
  const resolved = normalize(toplevel.stdout);
  if (!resolved || !samePath(resolved, path)) {
...
export function applyCleanup(plan, { dryRun = false } = {}) {
  ...
  for (const item of plan.items) {
    ...
    try {
      ...
      results.push({ ...item, result: "failed", error: safeError(error) });
    }
  }
  return results;
}
