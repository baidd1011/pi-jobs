...
import { atomicWriteJson, createJob, DATA_DIR, isValidJobId, loadConfig, readJob, withDataLock } from "./store.mjs";
...
function importResult(result) {
  if (!isValidJobId(result?.id) || readJob(result.id)) return false;
...
function importPending(task, running, defaults) {
  if (!isValidJobId(task?.id) || readJob(task.id)) return false;
