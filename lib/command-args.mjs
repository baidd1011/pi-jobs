...
export const NO_NETWORK_DEPRECATION = "--no-network is deprecated and will be removed in v2.0.0; use --local-tools-only";
export const DIGEST_NOTIFY_DEPRECATION = "/job digest --notify is deprecated and will be removed in v2.0.0; use plain digest or --markdown";
export const NS_DEPRECATION = "/ns is deprecated; use /job. /ns will be removed in v2.0.0";
...
export const ADD_USAGE = "usage: /job add <prompt> [--budget N] [--timeout MIN] [--max-turns N] [--tools read,edit,bash] [--local-tools-only] [--delivery branch|pr]";
...
export function parseAddArgs(text, defaults = {}) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === "--no-network" || arg === "--local-tools-only") {
      const canonical = "--local-tools-only";
      if (seen.has(canonical)) throw new Error(ADD_USAGE);
      seen.add(canonical);
      result.noNetwork = true;
      result.deprecatedNoNetwork = arg === "--no-network";
      continue;
    }
...
export function parseDigestArgs(text) {
  const args = tokens(text);
  const result = { hours: 24, markdown: false, notify: false, deprecatedNotify: false };
  const seen = new Set();
  for (let index = 0; index < args.length; index++) {
    const arg = args[index];
    ...
    if (arg === "--notify") { result.notify = true; result.deprecatedNotify = true; continue; }
    if (arg === "--hours") {
