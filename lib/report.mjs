...
export function nextAction(job) {
  if (!job) return null;
  if (job.statusDetail === "delivery-failed") {
    const branch = job.delivery?.branch || job.branch;
    const review = reviewCommand(job);
    const target = job.delivery?.target;
    const push = branch && job.repoRoot && target?.remoteName
      ? `git -C ${shellQuote(job.repoRoot)} push ${target.remoteName} refs/heads/${branch}:refs/heads/${branch}`
      : null;
    return [
      `Local result branch${branch ? ` ${branch}` : ""} is preserved.`,
      review ? `Review with: ${review}.` : "Review the local result before publishing.",
      push ? `If appropriate, push manually with: ${push}.` : "Push the preserved branch manually if appropriate.",
    ].join(" ");
  }
  if (job.statusDetail === "cleanup-needed") {
    const root = job.worktreePath || job.repoRoot;
    const inspect = root
      ? `git -C ${shellQuote(root)} status --short, then git -C ${shellQuote(root)} diff`
      : "inspect the preserved worktree status and diff";
    return `First ${inspect}. Then run /job cleanup --dry-run; cleanup never removes result branches or authoritative job records.`;
  }
  if (job.statusDetail === "worker-error") {
    return `Inspect /job log ${job.id} and /job audit ${job.id} before retrying.`;
  }
  return null;
}
...
export function buildAudit(jobId) {
  ...
  return {
    ...
    job: {
      ...
      error: job.error == null ? null : redactSensitive(job.error),
      ...
      delivery: sanitizeErrorFields({ delivery: job.delivery ?? null }).delivery,
...
function renderCleanupSection(digest) {
  ...
  const lines = digest.cleanupNeeded.map((entry) => `- \`${entry.id}\` **${entry.state}${entry.statusDetail ? ` (${entry.statusDetail})` : ""}** — branch: ${entry.delivery?.branch ? `\`${entry.delivery.branch}\`` : "_(none)_"}${entry.nextAction ? `\n  - Next: ${entry.nextAction}` : ""}`);
...
export function renderAuditMarkdown(audit) {
  ...
  lines.push(`**Effective policy:** tools \`${runtimeTools}\` — local-tools-only \`${job.runtime.policy.noNetwork}\` — max-turns \`${job.runtime.policy.maxTurns}\``);
  if (job.policy) lines.push(`**Requested policy:** tools \`${(job.policy.tools || []).join(",")}\` — local-tools-only \`${job.policy.noNetwork}\` — max-turns \`${job.policy.maxTurns}\``);
  ...
  if (audit.nextAction) lines.push(`**Next action:** ${audit.nextAction}`);
