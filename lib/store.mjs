...
function validateConfig(raw = {}) {
  ...
  const tools = Array.isArray(raw.tools) ? [...new Set(raw.tools.filter((tool) => allowedTools.has(tool)))] : DEFAULT_CONFIG.tools;
  const prRepositories = Array.isArray(raw.prRepositories) ? raw.prRepositories.filter((entry) => (
  ...
  return {
    ...
    tools: tools.length ? tools : [...DEFAULT_CONFIG.tools],
    prRepositories,
  };
}
...
export function saveConfig(raw) {
  ...
  atomicWriteJson(join(DATA_DIR, "config.json"), config);
  return config;
}
...
export function createJob(fields, { id = null, source = "pi-jobs" } = {}) {
  ...
  const job = {
    ...
    delivery: fields.delivery ?? { type: fields.deliveryType ?? "branch", status: "not-started", branch: null, commit: null },
...
export function reconcileQueueEvents() {
  return withDataLock("queue", () => {
    ...
    for (let revision = 1; revision <= state.revision; revision++) {
      ...
      const previous = [...events].reverse().find((event) => event.revision < revision);
      const next = events.find((event) => event.revision > revision);
      const event = {
        revision,
        from: previous?.to ?? "?",
        to: next?.from ?? { paused: state.paused },
        by: "recovery",
        reason: "queue:event-reconciled",
        at: state.updatedAt ?? state.resumedAt ?? state.pausedAt ?? new Date().toISOString(),
      };
      if (appendQueueEvent(event)) { events.push(event); events.sort((a, b) => a.revision - b.revision); repaired++; }
    }
    return repaired;
  });
}
